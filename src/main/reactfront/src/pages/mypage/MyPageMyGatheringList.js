...
function MyPageMyGatheringList() {
    ...
    const fetchData = () => {
        AuthAxios.get(`/api/mypage/my-gathering?page=${page}`)
            .then((response) => {
    ...
    const handleDelete = (gatheringId, memberId) => {
        let result = window.confirm("모임을 삭제하시겠습니까?");
        if (result) {
            AuthAxios.delete(`/api/gathering/${gatheringId}`)
                .then((response) => {
                            ...
                            <div className="my-gathering-list" key={number}>
                                ...
                                <div className="my-gathering-delete">
                                    <button className="my-gathering-delete-btn"
                                            onClick={() => handleDelete(item.gatheringId, memberId)}>삭제</button>
                                </div>
                            </div>
                        ))}
...
export default MyPageMyGatheringList;